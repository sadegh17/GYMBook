...
describe('fetchProgramTree', () => {
  it('queries program_days with nested items+exercise ordered by sort', async () => {
    const days = [{ id: 'd1', items: [], sections: [{ id: 'S1', sort: 0 }] }]
    const b = tail({ data: [{ ...days[0], sections: [{ id: 'S1', sort: 0 }] }], error: null })
...
describe('toggleCheck', () => {
  const exercise = { id: 'e1', met: 8, sec_per_rep: 4, name_fa: 'پرس سینه' }
  const item = { id: 'i1', sets: 3, reps: 12 }

  it('inserts exactly {user_id,date,day_key,item_id,kcal,exercise_name,sets} with correct kcal and returns {item_id,date}', async () => {
