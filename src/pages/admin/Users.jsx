...
function statusOf(p) {
  return p.status ?? (p.approved ? 'approved' : 'pending')
}

// Deletion is intentionally omitted: removing an auth user needs the
// service-role key (server-only). Rejection is the practical "block" path.
export default function Users() {
  ...
  // Real-time: refresh the list when any profile changes (new sign-ups appear
  // without a reload). Guarded so environments without a realtime client skip it.
  useEffect(() => {
    if (!isAdmin || typeof supabase?.channel !== 'function') return
  ...
  const approve = async (id, isSelf) => {
    if (isSelf) { setInlineMsg('نمی‌توانید دسترسی یا تایید حساب خودتان را تغییر دهید'); return }
    setInlineMsg('')
    try {
  ...
  const pendingList = all.filter((p) => statusOf(p) === 'pending')
  const rejectedList = all.filter((p) => statusOf(p) === 'rejected')

  const renderPending = () => {
    if (usersQuery.isLoading) return <div className="center muted" role="status">در حال بارگذاری…</div>
    ...
    return (
      <>
        <div className="card">
          ...
          {pendingList.length === 0 && <p className="muted">درخواست جدیدی وجود ندارد.</p>}
          {pendingList.map((p) => (
            <div key={p.id} className="user-row">
              ...
              <div className="user-actions">
                <button type="button" className="btn primary" onClick={() => approve(p.id, self?.id === p.id)}>تأیید</button>
                <button type="button" className="btn danger" onClick={() => reject(p.id, self?.id === p.id)}>رد</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="card">
            ...
            {rejectedList.map((p) => (
              <div key={p.id} className="user-row">
                <div className="user-head">
                  <b>{p.name}</b>
                  <span className="user-badge pending">رد شده</span>
                  <span className="email">{p.email}</span>
                </div>
                <div className="user-actions">
                  <button type="button" className="btn primary" onClick={() => approve(p.id, self?.id === p.id)}>تأیید</button>
  ...
  return (
    <div className="wrap">
      <div className="tabs" role="tablist" aria-label="بخش کاربران">
        <button type="button" role="tab" aria-selected={view === 'all'} className={view === 'all' ? 'tab active' : 'tab'} onClick={() => setView('all')}>
          کاربران
        </button>
        <button type="button" role="tab" aria-selected={view === 'pending'} className={view === 'pending' ? 'tab active' : 'tab'} onClick={() => setView('pending')}>
          در انتظار تأیید ({pendingList.length})
        </button>
      </div>
            ...
            {all.map((u) => (
              <div key={u.id} className="user-row">
                <div className="user-head">
                  ...
                  {!u.approved && statusOf(u) === 'pending' && <span className="user-badge pending">در انتظار تأیید</span>}
                  {statusOf(u) === 'rejected' && <span className="user-badge pending">رد شده</span>}
                ...
                <div className="user-fields">
                  ...
                  <label className="user-field check">
                    <input aria-label={`تأیید ${u.name}`} type="checkbox" checked={!!u.approved}
                      onChange={(e) => inline(u.id, { approved: e.target.checked, status: e.target.checked ? 'approved' : 'pending' }, self?.id === u.id)} />
